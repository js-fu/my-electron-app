...
The main-process HTTP client that backs the todo IPC handlers. It reads the
remote API base URL from the environment, issues CRUD requests against
`{API_BASE_URL}/todos` using Node.js built-in `fetch`, and falls back to a
seeded in-memory dev mock when no base URL is configured in development.
